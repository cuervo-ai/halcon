...
use halcon_core::types::OAuthConfig;
...
impl OAuthFlow {
    ...
    pub fn build_authorize_url(&self) -> Result<AuthorizeRequest> {
        ...
        let mut url = Url::parse(&self.config.authorize_url)
            .map_err(|e| HalconError::AuthFailed(format!("invalid authorize_url: {e}")))?;
    ...
    pub async fn exchange_code(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| HalconError::AuthFailed(format!("token exchange request failed: {e}")))?;

        let status = response.status();
        let body = response
            .text()
            .await
            .map_err(|e| HalconError::AuthFailed(format!("failed to read token response: {e}")))?;
        ...
        let token: TokenResponse = serde_json::from_str(&body)
            .map_err(|e| HalconError::AuthFailed(format!("invalid token response: {e}")))?;
    ...
    pub async fn create_api_key(&self, access_token: &str) -> Result<String> {
        let api_key_url = self
            ...
            .api_key_url
            .as_deref()
            .ok_or_else(|| HalconError::AuthFailed("no api_key_url configured".to_string()))?;
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                HalconError::AuthFailed(format!("API key creation request failed: {e}"))
            })?;

        let status = response.status();
        let body = response.text().await.map_err(|e| {
            HalconError::AuthFailed(format!("failed to read API key response: {e}"))
        })?;
        ...
        let parsed: serde_json::Value = serde_json::from_str(&body)
            .map_err(|e| HalconError::AuthFailed(format!("invalid API key response: {e}")))?;

        parsed["raw_key"]
            .as_str()
            .map(|s| s.to_string())
            .ok_or_else(|| {
                HalconError::AuthFailed("API key response missing 'raw_key' field".to_string())
            })
    }
}
