...
impl OAuthFlow {
    ...
    ///
    /// CSRF defense: `expected_state` (the value generated by
    /// [`Self::build_authorize_url`]) must match `received_state`
    /// (the value the authorization server returned in the redirect
    /// query string). The comparison runs in constant time. A
    /// mismatch is reported as an authentication failure and no
    /// token request is issued.
    pub async fn exchange_code(
        ...
        if expected_state.is_empty() {
            return Err(HalconError::AuthFailed(
                "OAuth expected_state is empty — refusing to exchange (no CSRF protection possible)"
                    .to_string(),
            ));
        }
        let eq: bool = expected_state
            .as_bytes()
...
mod tests {
    ...
    async fn exchange_code_rejects_state_mismatch() {
        ...
        let err = res.expect_err("state mismatch must reject");
        assert!(
    ...
    #[tokio::test]
    async fn exchange_code_rejects_empty_expected_state() {
        let config = OAuthConfig {
            client_id: "test-client".to_string(),
            authorize_url: "https://auth.example.com/authorize".to_string(),
            token_url: "https://auth.example.com/token".to_string(),
        ...
        let flow = OAuthFlow::new(config);
        let res = flow
            .exchange_code("auth-code-xyz", "verifier123", "", "")
            .await;
        let err = res.expect_err("empty expected_state must reject");
        assert!(err.to_string().contains("expected_state is empty"));
    }
    ...
    async fn exchange_code_error_response() {
        ...
        let flow = OAuthFlow::new(config);
        let result = flow
            .exchange_code("expired-code", "verifier123", "test-state", "test-state")
            .await;
    ...
    async fn exchange_code_network_error() {
        ...
        let flow = OAuthFlow::new(config);
        let result = flow
            .exchange_code("code", "verifier", "test-state", "test-state")
            .await;
