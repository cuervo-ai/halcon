...
fn render_event_row(ui: &mut egui::Ui, event: &WsServerEvent) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.add_space(2.0);
        ui.colored_label(HalconTheme::TEXT_SECONDARY, RichText::new(label).size(11.0));
    });
}
...
fn describe_event(event: &WsServerEvent) -> (&'static str, String, egui::Color32) {
    match event {
        ...
        WsServerEvent::AgentCompleted { id, success, .. } => {
            ...
            (
                icon,
                format!("Agent completed: {}", &id.to_string()[..8]),
                color,
            )
        }
        ...
        WsServerEvent::TaskCompleted {
            ...
            (
                icon,
                format!("Task completed: {}", &execution_id.to_string()[..8]),
                color,
            )
        }
        ...
        WsServerEvent::Metric(_) => ("~", "Metric point".to_string(), HalconTheme::TEXT_MUTED),
        WsServerEvent::Protocol(_) => {
            ("◈", "Protocol message".to_string(), HalconTheme::TEXT_MUTED)
        }
        ...
        WsServerEvent::SubAgentStarted {
            ...
            format!(
                "Sub-agent {}: {}",
                &sub_agent_id[..8.min(sub_agent_id.len())],
                task_description
            ),
            HalconTheme::ACCENT,
        ),
