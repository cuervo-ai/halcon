...
pub fn render_dag(ui: &mut Ui, nodes: &[DagNode]) {
    if nodes.is_empty() {
        ui.label(RichText::new("Empty DAG").color(HalconTheme::TEXT_MUTED));
        return;
    }
            ...
            for dep_id in &node.depends_on {
                if let Some(&from_pos) = positions.get(dep_id) {
                    ...
                    painter.line_segment(
                        ...
                        egui::Stroke::new(1.5, HalconTheme::BORDER),
                    );
                }
            }
        }
    ...
    for node in nodes {
        if let Some(&pos) = positions.get(&node.id) {
            ...
            let color = HalconTheme::task_status_color(&node.status);
...
pub fn nodes_from_task(task: &halcon_api::types::task::TaskExecution) -> Vec<DagNode> {
    task.node_results
        .iter()
        .enumerate()
        .map(|(i, nr)| DagNode {
