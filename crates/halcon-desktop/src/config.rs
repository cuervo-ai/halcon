...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            server_url: std::env::var("HALCON_SERVER_URL").unwrap_or_else(|_| {
                format!(
                    "http://{}:{}",
                    halcon_api::DEFAULT_BIND,
                    halcon_api::DEFAULT_PORT
                )
            }),
            auth_token: std::env::var("HALCON_API_TOKEN").unwrap_or_default(),
            dark_mode: true,
...
impl AppConfig {
    ...
    /// Path to the desktop config file: `~/.halcon/desktop.toml`.
    pub fn config_path() -> Option<PathBuf> {
        ...
        home.map(|h| PathBuf::from(h).join(".halcon").join("desktop.toml"))
    }

    /// Load config from `~/.halcon/desktop.toml`, falling back to defaults.
    ...
    pub fn load() -> Self {
        ...
        if let Ok(url) = std::env::var("HALCON_SERVER_URL") {
            config.server_url = url;
        }
        if let Ok(token) = std::env::var("HALCON_API_TOKEN") {
