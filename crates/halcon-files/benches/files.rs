...
fn bench_handler_helpers(c: &mut Criterion) {
    ...
    group.bench_function("estimate_tokens_1k", |b| {
        b.iter(|| halcon_files::handler::estimate_tokens_from_text(black_box(&text_1k)))
    });

    group.bench_function("estimate_tokens_100k", |b| {
        b.iter(|| halcon_files::handler::estimate_tokens_from_text(black_box(&text_100k)))
    });

    group.bench_function("truncate_no_truncation", |b| {
        b.iter(|| halcon_files::handler::truncate_to_budget(black_box(&text_1k), black_box(1000)))
    });

    group.bench_function("truncate_with_truncation", |b| {
        b.iter(|| halcon_files::handler::truncate_to_budget(black_box(&text_100k), black_box(100)))
    });
