...
impl Default for ConfidenceWeights {
    fn default() -> Self {
        ...
        Self { scope: 0.30, depth: 0.25, task_type: 0.25, language: 0.10, clarity: 0.10 }
    }
}
...
fn default_metacog_cycle() -> u32 { DEFAULT_METACOGNITIVE_CYCLE_ROUNDS }
...
mod tests {
    ...
    fn default_confidence_weights_sum_to_one() {
        ...
        assert!(w.is_valid(), "default weights must sum to 1.0, got {}",
    ...
    fn default_heuristics_matches_hardcoded_values() {
        ...
        assert_eq!(cfg.metacognitive_cycle_rounds, DEFAULT_METACOGNITIVE_CYCLE_ROUNDS);
    }
    ...
    fn heuristics_config_serde_roundtrip() {
        ...
        assert_eq!(parsed.word_count.single_artifact_max, cfg.word_count.single_artifact_max);
        assert_eq!(parsed.default_context_window, cfg.default_context_window);
    }

    #[test]
    fn custom_confidence_weights_validation() {
        let valid = ConfidenceWeights { scope: 0.5, depth: 0.2, task_type: 0.2, language: 0.05, clarity: 0.05 };
        assert!(valid.is_valid());

        let invalid = ConfidenceWeights { scope: 0.5, depth: 0.5, task_type: 0.5, language: 0.5, clarity: 0.5 };
    ...
    fn scope_confidences_all_in_unit_interval() {
        ...
        for val in [sc.conversational, sc.single_artifact, sc.local_context, sc.project_wide, sc.system_wide] {
            assert!(val >= 0.0 && val <= 1.0, "scope confidence out of [0,1]: {val}");
        }
    }
