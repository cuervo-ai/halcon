...
pub mod florence2 {
    ...
    pub async fn ground(_image_bytes: &[u8], _prompt: &str) -> Vec<BoundingBox> { vec![] }
}

/// Moshi real-time audio dialogue (Q2).
pub mod moshi {
    /// STUB: will stream audio tokens via WebSocket.
    pub async fn transcribe_streaming(_audio_chunk: &[u8]) -> Option<String> { None }
}
...
pub mod videorag {
    ...
    pub async fn extract_clips(_video_bytes: &[u8]) -> Vec<VideoClip> { vec![] }
}
...
pub mod imagebind {
    ...
    pub async fn embed(_data: &[u8], _modality: &str) -> Option<CrossModalEmbedding> { None }
}
