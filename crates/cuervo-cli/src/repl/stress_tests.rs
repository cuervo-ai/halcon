...
use cuervo_core::error::Result as CuervoResult;
...
use cuervo_core::types::ResilienceConfig;
use cuervo_tools::ToolRegistry;

use super::accumulator::CompletedToolUse;
use cuervo_core::types::Phase14Context;
...
impl ModelProvider for ToolCallProvider {
    ...
    async fn invoke(
    ...
    ) -> CuervoResult<BoxStream<'static, CuervoResult<ModelChunk>>> {
        ...
        if count == 0 {
            ...
            let chunks: Vec<CuervoResult<ModelChunk>> = vec![
...
fn test_event_tx() -> (cuervo_core::EventSender, cuervo_core::EventReceiver) {
...
fn make_tool_registry() -> ToolRegistry {
    ...
    cuervo_tools::default_registry(&config)
}
...
async fn fallback_provider_cost_tracking() {
    use cuervo_core::types::{BackpressureConfig, CircuitBreakerConfig};
    ...
    impl ModelProvider for CostProvider {
        ...
        async fn invoke(&self, request: &ModelRequest) -> CuervoResult<BoxStream<'static, CuervoResult<ModelChunk>>> {
        ...
        fn validate_model(&self, model: &str) -> CuervoResult<()> {
...
fn compaction_protocol_integrity() {
    use cuervo_core::types::CompactionConfig;
...
fn planning_gate_unsupported_model() {
    ...
    let provider2: Arc<dyn ModelProvider> = Arc::new(cuervo_providers::EchoProvider::new());
