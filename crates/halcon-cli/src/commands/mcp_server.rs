//! `halcon mcp-server` command: starts an MCP server over stdio.
//!
//! Spawned as a sidecar by IDEs (e.g., Tauri `externalBin`).
//! Reads JSON-RPC from stdin, exposes halcon tools, writes responses to stdout.
...
use halcon_mcp::McpServer;
use halcon_tools::default_registry;
