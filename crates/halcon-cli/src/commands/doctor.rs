//! `halcon doctor` — diagnostic command for runtime health.
//!
...
pub fn run(config: &AppConfig, db_path: Option<&Path>) -> Result<()> {
    ...
    let _ = writeln!(
        out,
        "\n  {muted}{tl}{h}{r} {primary}Halcon Doctor{r} {muted}{}{tr}{r}",
        h.repeat(38),
    );
...
fn print_config_section(config: &AppConfig, out: &mut impl Write) {
    use halcon_core::types::{validate_config, IssueLevel};
    let t = theme::active();
    let r = theme::reset();
...
fn assess_sync(
    db: &Database,
    provider: &str,
    config: &halcon_core::types::HealthConfig,
...
mod tests {
    ...
    fn doctor_with_seeded_metrics() {
        ...
        db.insert_resilience_event(&halcon_storage::ResilienceEvent {
            provider: "anthropic".to_string(),
    ...
    fn tool_metrics_section_with_data() {
        use chrono::Utc;
        use halcon_storage::ToolExecutionMetric;
    ...
    fn phase14_section_with_mcp_servers() {
        ...
        config.mcp.servers.insert(
            "test".to_string(),
            halcon_core::types::McpServerConfig {
