...
pub fn list(scope_filter: &str, working_dir: &Path) -> Result<()> {
    ...
    println!("{:<20}  {:<10}  Transport", "Name", "Scope");
    println!("{}", "-".repeat(60));
...
pub fn get(name: &str, working_dir: &Path) -> Result<()> {
    ...
    match &spec.transport {
        McpTransport::Http { url } => {
            ...
            let resolved_url = if let McpTransport::Http { url: u } = &expanded.transport {
                u.clone()
            } else {
                url.clone()
            };
...
pub async fn auth(name: &str, working_dir: &Path) -> Result<()> {
    ...
    let url = match &spec.transport {
        ...
        McpTransport::Stdio { .. } => {
            bail!("Server '{name}' uses stdio transport — OAuth not needed")
        }
    };
...
mod tests {
    ...
    fn auth_on_stdio_server_errors() {
        ...
        assert!(
            matches!(&spec.transport, McpTransport::Stdio { .. }),
