...
impl ModelSelector {
    ...
    fn select_by_strategy(
        ...
        match strategy {
            ...
            "fast" => {
                // Sort by observed p95 latency (ascending). Falls back to context_window
                // (larger = usually faster per token due to batching) when no hint is available.
                const DEFAULT_LATENCY_MS: u64 = 100;
                candidates.sort_by(|a, b| {
                    let lat_a = self.latency_hints.get(&a.id).copied().unwrap_or(DEFAULT_LATENCY_MS);
                    let lat_b = self.latency_hints.get(&b.id).copied().unwrap_or(DEFAULT_LATENCY_MS);
                    lat_a
                        .cmp(&lat_b)
