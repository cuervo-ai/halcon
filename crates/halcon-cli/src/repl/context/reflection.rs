    ...
    async fn gather(&self, query: &ContextQuery) -> Result<Vec<ContextChunk>> {
            ...
            if !trimmed.is_empty() {
                self.db
                    .search_memory_fts_by_type(
                        trimmed,
                        halcon_storage::MemoryEntryType::Reflection,
                        candidate_limit,
                    )
                    .await?
            } else {
        ...
        if entries.is_empty() {
            entries = self
                .db
                .list_memories(
                    Some(halcon_storage::MemoryEntryType::Reflection),
        ...
        let tokens = halcon_context::estimate_tokens(&content);
        Ok(vec![ContextChunk {
...
mod tests {
    ...
    async fn query_relevant_retrieval() {
        ...
        let r1 = halcon_storage::MemoryEntry {
            entry_id: uuid::Uuid::new_v4(),
            session_id: None,
            entry_type: halcon_storage::MemoryEntryType::Reflection,
        ...
        let r2 = halcon_storage::MemoryEntry {
            entry_id: uuid::Uuid::new_v4(),
            session_id: None,
            entry_type: halcon_storage::MemoryEntryType::Reflection,
