...
pub fn init_search_engine(db: Arc<Database>, config: SearchEngineConfig) {
    SEARCH_ENGINE.get_or_init(|| match SearchEngine::new(db, config) {
        Ok(engine) => {
...
pub async fn execute_search(
    query: &str,
) -> halcon_search::Result<halcon_search::types::SearchResults> {
    let engine_lock = get_search_engine().ok_or_else(|| {
        halcon_search::SearchError::ConfigError("Search engine not initialized".to_string())
    })?;
    ...
    let engine = engine_guard.as_ref().ok_or_else(|| {
        halcon_search::SearchError::ConfigError("Search engine initialization failed".to_string())
    })?;
