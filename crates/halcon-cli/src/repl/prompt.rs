...
/// Custom prompt for the halcon REPL.
///
/// Renders as: `halcon [sonnet] > `
pub struct HalconPrompt {
    left: String,
}
...
mod tests {
    ...
    fn shorten_sonnet() {
        assert_eq!(
            HalconPrompt::shorten_model("claude-sonnet-4-5-20250929"),
            "sonnet"
        );
    }
    ...
    #[test]
    fn shorten_opus() {
        assert_eq!(HalconPrompt::shorten_model("claude-opus-4-6"), "opus");
    }

    #[test]
    fn shorten_haiku() {
        assert_eq!(
            HalconPrompt::shorten_model("claude-haiku-4-5-20251001"),
            "haiku"
        );
    }
    ...
    #[test]
    fn shorten_gpt4o() {
        assert_eq!(HalconPrompt::shorten_model("gpt-4o"), "gpt-4o");
    }

    #[test]
    fn shorten_short_name() {
        assert_eq!(HalconPrompt::shorten_model("llama3.2"), "llama3.2");
    }

    #[test]
    fn shorten_long_name_truncates() {
        let long = "a-very-long-model-name-that-exceeds-twenty-chars";
        assert_eq!(HalconPrompt::shorten_model(long).len(), 20);
    }

    #[test]
    fn prompt_left_contains_model() {
        let p = HalconPrompt::new("anthropic", "claude-sonnet-4-5-20250929");
        assert_eq!(p.render_prompt_left().as_ref(), "halcon [sonnet]");
    }
}
