...
//!   → Evaluate policy chain (NonInteractive → PermissionLevel → PersistentRules → SessionMemory)
...
/// Checks persistent permission rules (Directory/Repository/Global scopes).
pub struct PersistentRulesPolicy {
    matcher: Arc<Mutex<RuleMatcher>>,
    #[allow(dead_code)]
    db: AsyncDatabase,
}

impl PersistentRulesPolicy {
    /// Create a new persistent rules policy.
    ///
    /// Loads rules from database asynchronously on first use (lazy init).
    pub fn new(matcher: Arc<Mutex<RuleMatcher>>, db: AsyncDatabase) -> Self {
        Self { matcher, db }
    }
}

impl AuthorizationPolicy for PersistentRulesPolicy {
    fn evaluate(
        &self,
        tool_name: &str,
        _perm_level: PermissionLevel,
        ...
        // Lock matcher for duration of match
...
impl AuthorizationMiddleware {
    ...
    /// Add persistent rules policy to the chain (before SessionMemoryPolicy).
    ///
    /// Inserts the policy at position 3 in the chain:
    /// CIDetection → NonInteractive → PermissionLevel → **PersistentRules** → SessionMemory
    pub fn with_persistent_rules(
        mut self,
        matcher: Arc<Mutex<RuleMatcher>>,
        db: AsyncDatabase,
    ) -> Self {
...
mod tests {
    ...
    fn persistent_rules_policy_integration() {
        ...
        // Add a global allow rule for bash
        ...
        async_db.inner().save_permission_rule(&rule).unwrap();

        // Load rules into matcher
        let rules = async_db.inner().load_all_permission_rules().unwrap();
    ...
    fn persistent_rules_policy_directory_scoped() {
        ...
        // Add a directory-scoped rule for file_write in /tmp
        ...
        let mut input = dummy_input(serde_json::json!({"path": "test.txt"}));
        input.working_directory = "/tmp".to_string();
        let state = AuthorizationState::new(true);
        let policy = PersistentRulesPolicy::new(matcher, async_db);
        let decision = policy.evaluate("file_write", PermissionLevel::Destructive, &input, &state);
        assert_eq!(decision, Some(PermissionDecision::AllowedForDirectory));
    }

    #[test]
    fn persistent_rules_policy_no_match_abstains() {
        use crate::repl::rule_matcher::RuleMatcher;
        use halcon_storage::Database;
        use std::path::PathBuf;
        use std::sync::{Arc, Mutex};
