...
const GENERAL_SIGNALS: SignalTable = &[
    ("hello", 0.5), ("hi", 0.5), ("hey", 0.5),
    ("what is", 0.7), ("what are", 0.7), ("how do", 0.6),
    ("explain", 0.8), ("tell me", 0.6), ("describe", 0.7),
    ("help", 0.4), ("question", 0.5), ("qué es", 0.7),
    ("cómo", 0.6), ("explica", 0.8), ("qué significa", 0.7),
];

const CODE_OPS_SIGNALS: SignalTable = &[
    ("implement", 0.9), ("create", 0.7), ("write", 0.7), ("build", 0.8),
    ("add function", 1.0), ("add method", 1.0), ("add class", 1.0),
    ("scaffold", 1.0), ("generate", 0.8), ("fix bug", 0.9), ("fix the", 0.5),
    ("refactor", 0.9), ("rewrite", 0.9), ("update", 0.6), ("modify", 0.7),
    ("debug", 0.9), ("patch", 0.8), ("compile", 0.8), ("run tests", 0.8),
    ("implementar", 0.9), ("crear", 0.7), ("escribir", 0.7), ("corregir", 0.9),
    ("refactorizar", 0.9), ("depurar", 0.9), ("compilar", 0.8),
];

const ARCH_SIGNALS: SignalTable = &[
    ("architecture", 1.0), ("design", 0.8), ("pattern", 0.7), ("structure", 0.8),
    ("component", 0.7), ("system design", 1.0), ("microservice", 1.0),
    ("module boundary", 1.0), ("coupling", 0.9), ("cohesion", 0.9),
    ("scalability", 0.9), ("distributed", 0.9), ("service mesh", 1.0),
    ("event-driven", 1.0), ("cqrs", 1.0), ("ddd", 1.0), ("hexagonal", 1.0),
    ("dependency", 0.7), ("abstraction", 0.8), ("layer", 0.6),
    ("arquitectura", 1.0), ("diseño del sistema", 1.0), ("microservicios", 1.0),
    ("acoplamiento", 0.9), ("escalabilidad", 0.9),
    // review/analysis framing
    ("review the architecture", 1.0), ("analyze the architecture", 1.0),
    ("architecture review", 1.0), ("design review", 0.9),
    ("revisar la arquitectura", 1.0), ("analizar la arquitectura", 1.0),
];

const SECURITY_SIGNALS: SignalTable = &[
    ("security", 1.0), ("vulnerability", 1.0), ("vulnerabilidad", 1.0),
    ("cve", 1.0), ("exploit", 1.0), ("attack", 0.9), ("threat", 0.9),
    ("injection", 1.0), ("sql injection", 1.0), ("xss", 1.0), ("csrf", 1.0),
    ("authentication", 0.9), ("authorization", 0.9), ("privilege", 0.9),
    ("penetration", 1.0), ("pentest", 1.0), ("owasp", 1.0),
    ("audit", 0.9), ("secure", 0.7), ("hardening", 1.0), ("encryption", 0.9),
    ("secret", 0.7), ("credentials", 0.8), ("token", 0.6), ("jwt", 0.9),
    ("seguridad", 1.0), ("vulnerabilidades", 1.0), ("brecha", 0.9),
    ("brechas de seguridad", 1.0), ("auditoría", 0.9), ("amenaza", 0.9),
    // explicit task framing
    ("security review", 1.0), ("security audit", 1.0), ("vulnerability scan", 1.0),
    ("revisión de seguridad", 1.0), ("auditoría de seguridad", 1.0),
    ("find vulnerabilities", 1.0), ("busca vulnerabilidades", 1.0),
    ("check security", 0.9), ("security analysis", 1.0),
];

const CODE_REVIEW_SIGNALS: SignalTable = &[
    ("review", 0.8), ("revisar", 0.8), ("code review", 1.0), ("revisión de código", 1.0),
    ("analyze", 0.7), ("analizar", 0.7), ("analyze the code", 1.0),
    ("inspect", 0.8), ("inspect the code", 1.0), ("code quality", 1.0),
    ("read the code", 0.9), ("leer el código", 0.9),
    ("look at the code", 0.8), ("check the code", 0.9),
    ("find issues", 0.9), ("find problems", 0.9), ("identify issues", 0.9),
    ("code smell", 1.0), ("technical debt", 1.0), ("best practices", 0.9),
    ("what does", 0.5), ("how does this work", 0.7), ("understand the code", 0.9),
    ("calidad del código", 1.0), ("deuda técnica", 1.0), ("buenas prácticas", 0.9),
    ("revisar el código", 1.0), ("analizar el código", 1.0),
    // repository-scope framing
    ("repository review", 1.0), ("repo review", 1.0), ("codebase review", 1.0),
    ("review the project", 1.0), ("review the codebase", 1.0),
    ("revisar el proyecto", 1.0), ("revisar el repositorio", 1.0),
    ("source code", 0.9), ("código fuente", 0.9),
];

const INFRA_SIGNALS: SignalTable = &[
    ("docker", 1.0), ("kubernetes", 1.0), ("k8s", 1.0), ("container", 0.9),
    ("deploy", 0.9), ("deployment", 0.9), ("ci/cd", 1.0), ("pipeline", 0.8),
    ("terraform", 1.0), ("ansible", 1.0), ("helm", 1.0), ("nginx", 0.9),
    ("load balancer", 1.0), ("autoscaling", 1.0), ("monitoring", 0.8),
    ("prometheus", 1.0), ("grafana", 1.0), ("logging", 0.7), ("metrics", 0.7),
    ("aws", 0.9), ("gcp", 0.9), ("azure", 0.9), ("cloud", 0.8),
    ("desplegar", 0.9), ("despliegue", 0.9), ("contenedor", 0.9),
];

const DATA_SIGNALS: SignalTable = &[
    ("database", 0.9), ("sql", 1.0), ("query", 0.7), ("migration", 1.0),
    ("schema", 0.9), ("table", 0.7), ("index", 0.7), ("join", 0.8),
    ("orm", 1.0), ("postgres", 1.0), ("mysql", 1.0), ("mongodb", 1.0),
    ("redis", 0.9), ("elasticsearch", 1.0), ("data pipeline", 1.0),
    ("etl", 1.0), ("spark", 1.0), ("kafka", 1.0), ("stream", 0.7),
    ("base de datos", 0.9), ("consulta", 0.7), ("migración", 1.0),
];
...
mod tests {
    ...
    fn secondary_detected_for_cross_domain() {
        ...
        assert!(d.secondary.is_some(), "expected secondary domain for mixed query");
    }
