...
use super::loop_state::{
    ExecutionIntentPhase, LoopState, SynthesisOrigin, SynthesisPriority, SynthesisTrigger,
};
...
pub(super) async fn run(
    ...
    if let Some(replay_exec) = replay_tool_executor {
        ...
        let all_tools = plan
            .parallel_batch
            .iter()
            .chain(plan.sequential_batch.iter());
        for tool_call in all_tools {
            let (content, is_error) = if let Some(recorded) = replay_exec.get_result(&tool_call.id)
            {
                ...
                (
                    format!(
                        "replay: no recorded result for tool_use_id '{}'",
                        tool_call.id
                    ),
                    true,
                )
        ...
        let (mut spec_hits, remaining_parallel): (Vec<executor::ToolExecResult>, Vec<_>) = {
            ...
            for tool_call in &plan.parallel_batch {
                if let Some(cached) = speculator
                    .get_cached(&tool_call.name, &tool_call.input)
                    .await
                {
                    tracing::debug!(tool = %tool_call.name, "Speculation cache hit");
                    if !state.silent {
                        render_sink.speculative_result(&tool_call.name, true);
                    }
        ...
        if !already_synthesizing
            && !state.silent
            && (!remaining_parallel.is_empty() || !plan.sequential_batch.is_empty())
        {
            render_sink.agent_state_transition(
                state.synthesis.phase_str(),
                "tool_wait",
                "executing tools",
            );
            state
                .synthesis
                .advance_phase(super::loop_state::AgentEvent::ToolsSubmitted);
        }
        ...
        if !state.silent && (!parallel_results.is_empty() || !sequential_results.is_empty()) {
            ...
            state
                .synthesis
                .advance_phase(super::loop_state::AgentEvent::ToolBatchComplete);
        }

        // Track tool invocations.
        session.tool_invocations += (parallel_results.len() + sequential_results.len()) as u32;
        ...
        let elider_budget = state
            .context_pipeline
            .accountant()
            .available(halcon_context::Tier::L0Hot)
            / 4;
        ...
        for result in parallel_results {
            ...
            } = block
            {
                *content = state.context_pipeline.elider().elide(
                ...
                state
                    .evidence
                    .bundle
                    .record_tool_result(&result.tool_name, content);
                ...
                state
                    .tool_trust
                    .record_success(&result.tool_name, result.duration_ms);
            }
            if let ContentBlock::ToolResult {
                ref content,
                is_error: true,
            ...
            } = block
            {
                ...
                state.tool_trust.record_failure(
                    &result.tool_name,
                    result.duration_ms,
                    Some(content.as_str()),
                );
    ...
    for (tool_name, error_content) in &tool_failures {
        state
            .guards
            .loop_guard
            .record_error(&format!("{}:{}", tool_name, error_content));
    }
    ...
    for (tool_name, error_content) in &tool_failures {
                ...
                .any(|(n, _)| n == tool_name)
        {
            ...
            state
                .evidence
                .blocked_tools
                .push((tool_name.clone(), short_reason));
            tracing::warn!(
                tool = %tool_name,
    ...
    if let Some(ref mut tracker) = state.execution_tracker {
        ...
        let matched = tracker.record_tool_results(&tool_successes, &failures_ref, round);
        ...
        state
            .guards
            .loop_guard
            .update_plan_progress(completed, total, elapsed);
        ...
        // large file generation tasks like the Minecraft benchmark).
        {
            ...
            let pending_are_all_synthesis = plan
                .steps
                .iter()
                .enumerate()
                .filter(|(_, s)| {
        ...
        state
            .tokens
            .tokens_per_round
            .push(state.tokens.call_input_tokens);
        ...
        let tokens_remaining = (state.tokens.provider_context_window as u64)
            .saturating_sub(state.tokens.call_input_tokens);
        ...
        if convergence_suppressed_by_truncation {
            ...
            if let Some(reason) = conv_result {
                let allowed = match &reason {
                    ...
                    ConvergenceReason::EvidenceThreshold => conv_policy
                        .as_ref()
                        .map_or(true, |p| p.allows_evidence_threshold(state.rounds as u32)),
                    ConvergenceReason::DiminishingReturns => conv_policy
                        .as_ref()
                        .map_or(true, |p| p.allows_diminishing_returns(state.rounds as u32)),
                };
                if allowed {
                    tracing::info!(
                    ...
                    if !state.silent {
                        render_sink.info(&format!("[convergence] {}", reason.description()));
                    }
                    state.guards.loop_guard.force_synthesis();
                } else {
                    tracing::info!(
        ...
        if let Some(ref mut bridge) = task_bridge {
            ...
            if bridge.is_strict() && bridge.has_permanently_failed_tasks() {
                ...
                render_sink.warning(
                    ...
                    Some(
                        "Use --full without --expert to allow continued execution on task failure",
                    ),
                );
            ...
            super::super::supervisor::BatchVerdict::ForceReplanNow(reason) => {
                ...
                let supervisor_replan_done = if let Some(planner_ref) = planner {
                    ...
                    if let Some(current_plan) = plan_clone {
                        match planner_ref
                            .replan(&current_plan, failed_idx, &reason, &request.tools)
                            .await
                        {
                            Ok(Some(new_plan)) => {
                                ...
                                {
                                    let report =
                                        state.convergence.coherence_checker.check(&new_plan);
                            ...
                            Ok(None) => {
                                tracing::warn!(
                                    "PostBatchSupervisor: supervisor replan returned no plan"
                                );
                                false
                            }
                            Err(e) => {
    ...
    if let Some(reflector) = reflector.filter(|_| !unified_eval_is_primary) {
        ...
        if should_reflect
            && !in_synthesizing
            && !matches!(outcome, super::super::reflexion::RoundOutcome::Success)
        {
            ...
            if !state.silent {
                ...
                state
                    .synthesis
                    .advance_phase(super::loop_state::AgentEvent::ReflectionComplete);
            }
    ...
    for (failed_tool_name, error_msg) in &tool_failures {
        let (count, tripped) = state
            .guards
            .failure_tracker
            .record_with_count(failed_tool_name, error_msg);
        ...
        if tripped {
            ...
            if !state.silent {
                render_sink.loop_guard_action(
                    "circuit_breaker",
                    &format!("{failed_tool_name}: repeated failures"),
                );
            }
            ...
            for (failed_tool_name, error_msg) in &tool_failures {
                if pr.plugin_id_for_tool(failed_tool_name).is_some() {
                    ...
                    state
                        .guards
                        .failure_tracker
                        .record(failed_tool_name, pattern);
                }
            }
        } // end lock
    }
    ...
    if let (Some(ref mut tracker), Some(planner)) = (&mut state.execution_tracker, planner) {
        for (failed_tool_name, error_msg) in &tool_failures {
            ...
            if state
                .guards
                .failure_tracker
                .is_tripped(failed_tool_name, error_msg)
            {
                tracing::info!(
                    tool = %failed_tool_name,
    ...
    {
        let web_search_no_results = state.messages.last().is_some_and(|msg| {
    ...
    let cycle_calls: Vec<(String, String)> = round_tool_log
        .iter()
    ...
    if state
        ...
        .record_round(round, &cycle_calls)
    {
        tracing::info!(
            round,
