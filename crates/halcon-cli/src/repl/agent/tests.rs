    ...
    fn test_ctx<'a>(
        ...
        AgentContext {
            ...
            // See comment above (FASE H) for rationale.
            speculator: Box::leak(Box::new(crate::repl::tool_speculation::ToolSpeculator::new())),
    ...
    fn intent_scorer_profile_compatible_with_reasoning_panel() {
        ...
        for query in &queries {
            ...
            assert!(
                ...
                "suggested_max_rounds must be positive for {:?}", query
            );
        }
    }
    ...
    fn convergence_action_exhaustive_match() {
        ...
        assert_eq!(actions.len(), 4, "ConvergenceAction must have exactly 4 variants");
    }
    ...
    fn intent_scorer_is_deterministic_for_same_input() {
        ...
        assert_eq!(p1.task_type, p2.task_type, "task_type must be deterministic");
        assert_eq!(p1.complexity, p2.complexity, "complexity must be deterministic");
    ...
    fn intent_scorer_handles_empty_string_without_panic() {
        ...
        assert!(profile.suggested_max_rounds() > 0, "empty input must still produce a valid budget");
        assert!(!profile.task_hash.is_empty(), "task_hash must be non-empty even for empty input");
    }
    ...
    fn intent_scorer_single_word_is_conversational_or_simple() {
        ...
        for word in &["hola", "help", "quit", "hi"] {
            ...
            assert!(
                ...
                "{:?} → complexity {:?} (expected Simple)", word, p.complexity
            );
            assert!(
                p.suggested_max_rounds() <= 4,
                "{:?} → max_rounds {} (expected ≤ 4)", word, p.suggested_max_rounds()
            );
        }
    }
    ...
    fn intent_scorer_very_long_query_does_not_panic() {
        ...
        assert!(p.suggested_max_rounds() > 0, "long query must produce valid budget");
        assert!(
            p.complexity == TaskComplexity::Complex || p.complexity == TaskComplexity::Moderate,
            "long query should be Complex or Moderate, got {:?}", p.complexity
        );
    }
    ...
    fn intent_scorer_handles_non_alphabetic_input() {
        ...
        for input in &inputs {
            ...
            assert!(p.suggested_max_rounds() > 0, "input {:?} must produce valid budget", input);
        }
    }
    ...
    fn convergence_controller_max_rounds_getter_matches_intent_profile() {
        ...
        for query in &[
            ...
            assert!(
                ...
                query, ctrl.max_rounds(), profile.suggested_max_rounds()
            );
        }
    }
    ...
    fn fresh_speculator_has_clean_state() {
        ...
        assert_eq!(metrics.total_checks, 0, "fresh speculator must have zero total_checks");
    ...
    fn phase_l_regression_budget_invariant_analiza_scenario() {
        use crate::repl::plan_state_diagnostics::BudgetInvariantChecker;
        ...
        assert!(result.is_err(), "K5-1 must fire: max_rounds=2 for 2-step plan + 1 retry");
