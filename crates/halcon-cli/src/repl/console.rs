...
use halcon_core::types::OrchestratorResult;
use halcon_storage::{CacheStats, MemoryStats, SystemMetrics, TraceStep, TraceStepType};
...
pub fn render_metrics(
    ...
    speculation: Option<&super::tool_speculation::SpeculationMetricsSnapshot>,
    out: &mut impl Write,
) {
    ...
    if let Some(spec) = speculation {
        ...
        let badge_color = if spec.hit_rate >= 0.5 {
            t.palette.success.fg()
        } else if spec.hit_rate >= 0.2 {
        ...
        let _ = writeln!(
            out,
            "      Checks: {}, Hits: {cached_color}{}{r}, Misses: {}, Hit Rate: {badge_color}{hit_rate}{r}, Latency Saved: {cached_color}{latency}{r}",
            spec.total_checks, spec.hits, spec.misses
        );
    }
}
...
mod tests {
    ...
    fn render_metrics_empty() {
        ...
        render_metrics(&sys, None, None, None, &mut buf);
        let output = String::from_utf8(buf).unwrap();
    ...
    fn render_metrics_populated() {
        ...
        let sys = SystemMetrics {
            ...
            total_tokens: 10000,
            models: vec![halcon_storage::ModelStats {
                provider: "echo".into(),
                model: "echo".into(),
                total_invocations: 50,
        ...
        render_metrics(&sys, Some(&cache), Some(&memory), None, &mut buf);
        let output = String::from_utf8(buf).unwrap();
