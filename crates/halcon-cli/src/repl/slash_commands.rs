        ...
        match target {
            ...
            commands::InspectTarget::SdlcServers => {
                ...
                if let Some(ref cm) = self.context_manager {
                    ...
                    if sdlc_servers.is_empty() {
                        let _ = writeln!(out, "No SDLC context servers detected.");
                        let _ = writeln!(out, "  Expected: RequirementsServer, ArchitectureServer, CodebaseServer, etc.");
                    } else {
                        let _ = writeln!(out, "Active servers: {}/{}", sdlc_servers.len(), 8);
                        for (name, priority) in sdlc_servers {
                            let _ = writeln!(out, "  {} (priority: {})", name, priority);
                        }
    ...
    pub(crate) async fn handle_benchmark(&self, workload: &str) {
        ...
        match workload {
            ...
            "cache" => {
                ...
                let stats = db.cache_stats().unwrap_or(halcon_storage::CacheStats {
                    total_entries: 0,
                    total_hits: 0,
                    oldest_entry: None,
                    newest_entry: None,
