...
pub enum InspectTarget {
    ...
    CostMetrics,
    /// SDLC context servers state (8 servers: requirements, architecture, etc.).
    SdlcServers,
}
...
fn handle_inspect(args: &str) -> CommandResult {
    match args.trim() {
        ...
        "sdlc" | "servers" => CommandResult::Inspect(InspectTarget::SdlcServers),
        "" => {
...
CLI Commands (run from shell):
  halcon chat \"msg\"   Single-shot mode
  halcon chat -r ID   Resume a session
  halcon doctor       Runtime health diagnostics
  halcon auth login   Configure API keys"
    );
}
