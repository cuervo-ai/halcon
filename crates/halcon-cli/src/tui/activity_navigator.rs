...
pub struct ActivityNavigator {
    ...
    /// Phase 3 SRCH-004: Search history buffer (loaded from DB on init).
    /// Stores recent unique queries for Up/Down arrow navigation.
    pub search_history: Vec<String>,

    /// Phase 3 SRCH-004: Current position in search history.
    /// 0 = most recent, increments with Up arrow, decrements with Down.
    pub history_index: usize,

    /// Phase 3 SRCH-004: Temporary query being typed before history navigation.
    /// Saved when user presses Up, restored when pressing Down past history end.
...
impl ActivityNavigator {
    ...
    // Phase 3 SRCH-004: Search history navigation

    /// Load search history from database (called at TUI init).
    ///
    /// ## Arguments
    /// - `history`: Vec of recent query strings (most recent first)
    pub fn load_history(&mut self, history: Vec<String>) {
        self.search_history = history;
        self.history_index = 0;
    }

    /// Navigate up in search history (older queries).
    ///
    /// ## Behavior
    /// - On first Up: saves current input, loads most recent history entry
    /// - On subsequent Up: moves to older entries (index++)
    /// - Returns new query to display, or None if at end of history
    pub fn history_up(&mut self) -> Option<String> {
        ...
        if self.history_index == 0 && !self.search_query.is_empty() {
            self.current_input = self.search_query.clone();
        }

        if self.history_index < self.search_history.len() {
            let query = self.search_history[self.history_index].clone();
    ...
    /// Navigate down in search history (newer queries).
    ///
    /// ## Behavior
    /// - Moves to newer entries (index--)
    /// - If reaching index 0: restores original current_input
    /// - Returns new query to display, or None if already at newest
    pub fn history_down(&mut self) -> Option<String> {
        if self.history_index == 0 {
            return None; // Already at newest (current input)
        }
        ...
        if self.history_index == 0 {
            ...
            // Still in history
            Some(self.search_history[self.history_index - 1].clone())
        }
    }

    /// Reset history navigation state (called when search overlay closes).
    pub fn reset_history_nav(&mut self) {
        self.history_index = 0;
        self.current_input.clear();
    }

    /// Push a query to history buffer (in-memory).
    ///
    /// ## Behavior
    /// - Deduplicates: if query already exists, removes old occurrence
    /// - Inserts at front (most recent)
    /// - Keeps max 50 entries in memory
    pub fn push_to_history(&mut self, query: String) {
        if query.is_empty() {
            return;
        }
        ...
        // Limit to 50 in-memory entries (DB handles long-term storage)
...
mod tests {
    ...
    // Phase 3 SRCH-004: Search history navigation tests
    ...
    fn load_history_stores_queries() {
        ...
        assert_eq!(nav.search_history, history);
    ...
    fn history_up_navigates_to_older_queries() {
        ...
        let q1 = nav.history_up();
        assert_eq!(q1, Some("recent".to_string()));
        assert_eq!(nav.current_input, "current typing"); // Saved
        assert_eq!(nav.history_index, 1);

        // Second Up: returns older
        let q2 = nav.history_up();
        assert_eq!(q2, Some("older".to_string()));
        assert_eq!(nav.history_index, 2);

        // Third Up: returns oldest
        let q3 = nav.history_up();
        assert_eq!(q3, Some("oldest".to_string()));
        assert_eq!(nav.history_index, 3);

        // Fourth Up: at end, returns None
        let q4 = nav.history_up();
        assert_eq!(q4, None);
        assert_eq!(nav.history_index, 3); // Stays at end
    }

    #[test]
    fn history_down_navigates_to_newer_queries() {
        let mut nav = ActivityNavigator::new();
        nav.load_history(vec!["recent".to_string(), "older".to_string()]);
        nav.search_query = "current".to_string();

        // Navigate up twice
        nav.history_up(); // "recent", index=1
        nav.history_up(); // "older", index=2

        // Down: back to "recent"
        let q1 = nav.history_down();
        assert_eq!(q1, Some("recent".to_string()));
        assert_eq!(nav.history_index, 1);

        // Down again: back to current input
        let q2 = nav.history_down();
        assert_eq!(q2, Some("current".to_string()));
        assert_eq!(nav.history_index, 0);

        // Down again: already at newest, returns None
        let q3 = nav.history_down();
        assert_eq!(q3, None);
        assert_eq!(nav.history_index, 0);
    }
    ...
    fn history_empty_returns_none() {
        ...
        assert_eq!(nav.history_up(), None);
        assert_eq!(nav.history_down(), None);
    }

    #[test]
    fn push_to_history_deduplicates() {
        let mut nav = ActivityNavigator::new();

        nav.push_to_history("query1".to_string());
        nav.push_to_history("query2".to_string());
        nav.push_to_history("query1".to_string()); // Duplicate

        // "query1" should be at front, "query2" second (old "query1" removed)
        assert_eq!(nav.search_history.len(), 2);
        assert_eq!(nav.search_history[0], "query1");
        assert_eq!(nav.search_history[1], "query2");
    }

    #[test]
    fn push_to_history_limits_to_50() {
        let mut nav = ActivityNavigator::new();

        // Push 60 queries
        for i in 0..60 {
            nav.push_to_history(format!("query{}", i));
        }

        assert_eq!(nav.search_history.len(), 50);
        assert_eq!(nav.search_history[0], "query59"); // Most recent
        assert_eq!(nav.search_history[49], "query10"); // 50th entry
    }

    #[test]
    fn push_to_history_ignores_empty() {
        let mut nav = ActivityNavigator::new();

        nav.push_to_history("query1".to_string());
        nav.push_to_history("".to_string()); // Empty, should be ignored

        assert_eq!(nav.search_history.len(), 1);
        assert_eq!(nav.search_history[0], "query1");
    }

    #[test]
    fn reset_history_nav_clears_state() {
        let mut nav = ActivityNavigator::new();
        nav.load_history(vec!["query1".to_string()]);
        nav.history_up(); // index=1, current_input saved

        nav.reset_history_nav();

        assert_eq!(nav.history_index, 0);
        assert_eq!(nav.current_input, "");
    }

    #[test]
    fn history_preserves_current_input_across_navigation() {
        let mut nav = ActivityNavigator::new();
        nav.load_history(vec!["old query".to_string()]);
        nav.search_query = "new typing".to_string();

        // Up: saves "new typing"
        nav.history_up();
        assert_eq!(nav.current_input, "new typing");

        // Down: restores "new typing"
        let restored = nav.history_down();
        assert_eq!(restored, Some("new typing".to_string()));
    }
}
