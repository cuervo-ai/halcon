...
// --- Compact Prompt Constants (Phase I2) ---
/// Base height: 1 line for input + 1 separator line.
const PROMPT_BASE_HEIGHT: u16 = 1;
/// Separator line height (subtle bottom border).
const PROMPT_SEPARATOR_HEIGHT: u16 = 1;
/// Absolute maximum prompt height (prevents excessive viewport consumption).
const PROMPT_MAX_HEIGHT: u16 = 12;
/// Legacy max fraction (deprecated, replaced by PROMPT_MAX_HEIGHT).
#[allow(dead_code)]
const PROMPT_MAX_FRACTION: f32 = 0.30;

/// Calculate dynamic prompt height based on content lines.
///
/// Formula: BASE (1) + SEPARATOR (1) + content_lines
/// Clamped: [2, PROMPT_MAX_HEIGHT]
///
/// # Phase I2 Compact Prompt
/// - Empty: 2 lines (1 input + 1 separator)
/// - Single line: 2 lines
/// - Multi-line: 2 + (n-1) lines, max 12 total
pub fn calculate_prompt_height(content_lines: usize, area_height: u16) -> u16 {
    let content_height = if content_lines <= 1 {
        PROMPT_BASE_HEIGHT
    } else {
        // For n lines of content, we need n lines of display
        content_lines as u16
    };

    let total = content_height + PROMPT_SEPARATOR_HEIGHT;

    // Clamp: minimum 2 (1 input + 1 separator), maximum PROMPT_MAX_HEIGHT
    // Also ensure we don't exceed 30% of available height
    let max_allowed = PROMPT_MAX_HEIGHT.min((area_height as f32 * 0.30) as u16);
    total.max(2).min(max_allowed)
}

/// Calculate the 3-zone layout from the available terminal area.
///
/// # Phase I2
/// Uses dynamic prompt height with default 1 content line.
/// For dynamic resizing based on actual content, use calculate_zones_dynamic().
pub fn calculate_zones(area: Rect) -> Zones {
    calculate_zones_dynamic(area, 1)
}

/// Calculate the 3-zone layout with dynamic prompt height.
///
/// # Arguments
/// * `area` - Available terminal area
/// * `content_lines` - Number of lines in prompt content (0 = empty, 1+ = actual lines)
pub fn calculate_zones_dynamic(area: Rect, content_lines: usize) -> Zones {
...
/// Default side panel width percentage (reduced from 20 to 15 for more Activity space).
const PANEL_WIDTH_PCT: u16 = 15;
...
pub fn calculate_cockpit_zones(area: Rect, config: &LayoutConfig) -> CockpitZones {
    calculate_cockpit_zones_dynamic(area, config, 1)
}
...
pub fn calculate_cockpit_zones_dynamic(area: Rect, config: &LayoutConfig, content_lines: usize) -> CockpitZones {
    ...
    // Prompt at bottom (dynamic height)
    let prompt_height = calculate_prompt_height(content_lines, area.height)
        .min(area.height.saturating_sub(status_height + 4));
...
pub fn calculate_mode_layout(area: Rect, mode: UiMode, panel_visible: bool) -> ModeLayout {
    calculate_mode_layout_dynamic(area, mode, panel_visible, 1)
}
...
mod tests {
    ...
    fn zones_standard_terminal() {
        ...
        assert!(zones.prompt.height >= 2, "compact prompt min 2 lines");
    ...
    // --- Phase I2: Compact Prompt Dynamic Height Tests ---

    #[test]
    fn prompt_height_empty_is_2() {
        let height = calculate_prompt_height(0, 40);
        assert_eq!(height, 2, "empty prompt: 1 base + 1 separator");
    }

    #[test]
    fn prompt_height_single_line_is_2() {
        let height = calculate_prompt_height(1, 40);
        assert_eq!(height, 2, "single line: 1 content + 1 separator");
    }
    ...
    #[test]
    fn prompt_height_respects_30_percent_limit() {
        // Area height 40 → 30% = 12
        let height = calculate_prompt_height(20, 40);
        assert_eq!(height, 12, "30% of 40 = 12, even though content wants 21");
    }

    #[test]
    fn prompt_height_small_terminal_proportional() {
        // Area height 20 → 30% = 6
        let height = calculate_prompt_height(10, 20);
        assert_eq!(height, 6, "small terminal: 30% of 20 = 6");
    }

    #[test]
    fn zones_dynamic_single_line() {
        let area = Rect::new(0, 0, 120, 40);
        let zones = calculate_zones_dynamic(area, 1);
        assert_eq!(zones.prompt.height, 2, "1 line content → 2 total");
    }

    #[test]
    fn zones_dynamic_multi_line() {
        let area = Rect::new(0, 0, 120, 40);
        let zones = calculate_zones_dynamic(area, 5);
        assert_eq!(zones.prompt.height, 6, "5 lines content → 6 total");
    }
    ...
    #[test]
    fn mode_layout_dynamic_compact() {
        let area = Rect::new(0, 0, 120, 40);
        let layout = calculate_mode_layout_dynamic(area, UiMode::Minimal, false, 1);
        assert_eq!(layout.prompt.height, 2, "1 line → compact 2");
    }

    #[test]
    fn mode_layout_dynamic_expands() {
        let area = Rect::new(0, 0, 120, 40);
        let layout = calculate_mode_layout_dynamic(area, UiMode::Minimal, false, 8);
        assert_eq!(layout.prompt.height, 9, "8 lines → 9 total");
    }

    #[test]
    fn cockpit_zones_dynamic_compact() {
        let area = Rect::new(0, 0, 120, 40);
        let config = LayoutConfig::default();
        let zones = calculate_cockpit_zones_dynamic(area, &config, 1);
        assert_eq!(zones.prompt.height, 2, "compact at 1 line");
    }

    #[test]
    fn cockpit_zones_dynamic_multi() {
        let area = Rect::new(0, 0, 120, 40);
        let config = LayoutConfig::default();
        let zones = calculate_cockpit_zones_dynamic(area, &config, 4);
        assert_eq!(zones.prompt.height, 5, "4 lines content + separator");
    }
}
