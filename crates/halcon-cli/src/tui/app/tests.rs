...
fn stream_thinking_accumulates_silently_without_flooding() {
    ...
    assert_eq!(
        app.state.thinking_buffer, expected,
        "Buffer should accumulate all fragments"
    );
    ...
    let info_lines = app
        ...
        .all_lines()
        .iter()
        .filter(|l| matches!(l, crate::tui::activity_types::ActivityLine::Info(_)))
        .count();
...
fn stream_thinking_collapsed_on_first_stream_chunk() {
    ...
    assert!(
        app.state.thinking_buffer.is_empty(),
    ...
    let has_answer = app.activity_model.all_lines().iter().any(|l| {
        if let crate::tui::activity_types::ActivityLine::AssistantText(t) = l {
            t.contains("Final answer")
        } else {
            false
        }
    });
...
fn stream_thinking_collapsed_on_done_when_no_stream_chunk() {
    ...
    assert!(
        has_response,
        "Thinking content surfaced as assistant response"
    );
}
...
fn push_banner_with_routing_shows_chain() {
    ...
    let routing = RoutingDisplay {
        ...
        fallback_chain: vec!["anthropic".into(), "deepseek".into(), "ollama".into()],
    };
    let features = crate::render::banner::FeatureStatus::default();
...
fn input_always_enabled_regardless_of_focus() {
    ...
    assert_eq!(
        text, "h",
        "Input should ALWAYS be enabled, even when focus is on Activity"
    );

    // Verify typing indicator is active.
    assert!(
        app.state.typing_indicator,
        "Typing indicator should activate when user types"
    );
}
...
fn navigation_keys_respect_focus() {
    ...
    assert!(
        app.activity_navigator.auto_scroll,
        "Arrow keys should not scroll Activity when focus is on Prompt"
    );
    ...
    assert!(
        !app.activity_navigator.auto_scroll,
        "Arrow keys should scroll Activity when focus is there"
    );
}
...
fn approve_sends_on_perm_channel() {
    ...
    assert_eq!(
        ...
        halcon_core::types::PermissionDecision::Allowed
    );
}
...
fn slash_plan_switches_panel_to_plan() {
    ...
    assert!(app.state.panel_visible);
    assert_eq!(
        app.state.panel_section,
        crate::tui::state::PanelSection::Plan
    );
}
...
fn event_summary_exhaustive() {
    ...
    let events: Vec<UiEvent> = vec![
        ...
        UiEvent::ProviderFallback {
            from: "a".into(),
            to: "b".into(),
            reason: "r".into(),
        },
        UiEvent::LoopGuardAction {
            action: "a".into(),
            reason: "r".into(),
        },
        ...
        UiEvent::SubAgentSpawned {
            step_index: 1,
            total_steps: 3,
            description: "Analyse project".into(),
        ...
        UiEvent::ProjectConfigCreated {
            path: "/tmp/myproject/.halcon/HALCON.md".into(),
        },
        UiEvent::ProjectHealthCalculated {
            score: 78,
            issues: vec!["No CI detected".into()],
            recommendations: vec!["Add GitHub Actions".into()],
        },
        ...
        UiEvent::PluginBootstrapStarted {
            count: 2,
            dry_run: false,
        },
...
fn cancel_mid_animation_reverses_direction() {
    ...
    assert!(
        (current - midpoint).abs() < 0.1,
        "Should start from midpoint, got {}",
        current
    );
}
...
fn ease_in_out_symmetry() {
    ...
    assert!(
        (mid - 0.5).abs() < 0.1,
        "Midpoint easing should be ~0.5, got {}",
        mid
    );
}
...
fn shimmer_progress_cycles_at_one_second() {
    ...
    assert!(
        ...
        "Shimmer should cycle at 1s, got {}",
        progress
    );
}
...
fn shimmer_progress_midpoint() {
    ...
    assert!(
        ...
        "Shimmer at 500ms should be ~0.5, got {}",
        progress
    );
}
...
fn shimmer_progress_quarter() {
    ...
    assert!(
        ...
        "Shimmer at 250ms should be ~0.25, got {}",
        progress
    );
}
...
fn shimmer_progress_repeats_after_cycle() {
    ...
    assert!(
        (p1 - p2).abs() < 0.01,
        "Shimmer should repeat after 1s cycle"
    );
}
...
fn search_next_adds_highlight() {
    ...
    assert!(
        highlights.is_pulsing(&highlight_key),
        "Highlight should be active after search_next"
    );
}
...
fn search_highlight_stops_after_navigation() {
    ...
    assert!(
        !highlights.is_pulsing(&old_key),
        "Old highlight should be stopped"
    );
    assert!(
        highlights.is_pulsing(&new_key),
        "New highlight should be active"
    );
}
...
fn rerun_search_highlights_first_match() {
    ...
    assert!(
        app.highlights.is_pulsing(highlight_key),
...
fn hover_cleared_when_mouse_leaves() {
    ...
    assert_eq!(
        nav.hovered(),
        None,
        "Hover should be cleared when mouse leaves"
    );
}
...
fn search_history_resets_on_overlay_close() {
    ...
    // Pre-load history
    app.activity_navigator
        .load_history(vec!["test".to_string()]);

    // Open search overlay and navigate history
    app.state.overlay.open(OverlayKind::Search);
    let _ = app.activity_navigator.history_up();
    assert!(
        app.activity_navigator.history_index > 0,
        "History index should be > 0 after navigation"
    );
    ...
    assert_eq!(
        app.activity_navigator.history_index, 0,
        "History index should be reset to 0"
    );
}
