...
impl ActivityModel {
    ...
    /// Find the index of the most recent PlanOverview line, if any.
    pub fn find_plan_overview_idx(&self) -> Option<usize> {
        self.lines
            .iter()
            .enumerate()
            .rev()
    ...
    /// Find all ToolExec line indices for a given tool name.
    pub fn find_tool_indices(&self, tool_name: &str) -> Vec<usize> {
        self.lines
            .iter()
            .enumerate()
            .filter_map(|(idx, line)| {
    ...
    /// Push a transient "thinking" skeleton indicator (idempotent — no duplicate added).
    ///
