...
//! End-to-end integration tests for the `halcon` binary.
//!
...
///
/// Sets HALCON_CONFIG to a temp config file so no real home files are touched.
...
fn version_flag() {
    ...
    halcon_cmd(&tmp)
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("halcon"));
}
...
fn env_var_overrides_provider() {
    let tmp = TempDir::new().unwrap();
    halcon_cmd(&tmp)
        .env("HALCON_PROVIDER", "echo")
        .env("HALCON_MODEL", "echo")
...
fn verbose_flag_enables_debug_output() {
    let tmp = TempDir::new().unwrap();
    halcon_cmd(&tmp)
        .env_remove("HALCON_LOG") // allow --verbose to take effect
...
fn init_creates_project_config() {
    ...
    // Verify .halcon/ directory was created.
    assert!(tmp.path().join(".halcon").exists());
}
