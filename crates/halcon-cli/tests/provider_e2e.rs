...
enabled = true
api_base = "{base_url}"
api_key_env = "HALCON_TEST_API_KEY"
...
async fn anthropic_mock_single_shot() {
    ...
    halcon_cmd(&tmp)
        .env("HALCON_TEST_API_KEY", "sk-ant-api03-test-key")
        .args([
            "--config",
...
async fn anthropic_mock_sends_correct_headers() {
    ...
    halcon_cmd(&tmp)
        .env("HALCON_TEST_API_KEY", "sk-ant-api03-verify-headers")
        .args([
            "--config",
            config_path.to_str().unwrap(),
            "-p",
...
async fn anthropic_mock_auth_error() {
    ...
    halcon_cmd(&tmp)
        .env("HALCON_TEST_API_KEY", "sk-ant-api03-bad-key")
        .args([
            "--config",
            config_path.to_str().unwrap(),
            "-p",
...
async fn anthropic_mock_rate_limit() {
    ...
    halcon_cmd(&tmp)
        .env("HALCON_TEST_API_KEY", "sk-ant-api03-test")
        .args([
            "--config",
            config_path.to_str().unwrap(),
            "-p",
...
async fn anthropic_auth_error_not_retried() {
    ...
    halcon_cmd(&tmp)
        .env("HALCON_TEST_API_KEY", "sk-ant-api03-bad")
        .args([
            "--config",
            config_path.to_str().unwrap(),
            "-p",
