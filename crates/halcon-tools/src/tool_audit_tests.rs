...
mod audit {
    ...
    mod web_search_audit {
        ...
        async fn empty_index_returns_no_results() {
            ...
            match result {
                Ok(out) => {
                    assert!(!out.is_error);
                    assert!(out.content.contains("No results") || out.content.contains("empty"));
                }
                Err(_) => {} // database not initialized is also acceptable
            }
        }
    ...
    mod destructive_hardening {
        ...
        async fn file_write_no_temp_file_left_behind() {
            // After a successful write, no .halcon_tmp_ files should remain.
            let dir = tempfile::TempDir::new().unwrap();
            let tool = FileWriteTool::new(test_fs());
