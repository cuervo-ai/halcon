...
impl Database {
    ...
    pub fn get_media_cache(&self, content_hash: &str) -> Result<Option<MediaCacheEntry>> {
        ...
        ).optional().map_err(|e| HalconError::DatabaseError(e.to_string()))
    }
    ...
    pub fn store_media_cache(&self, entry: &MediaCacheEntry) -> Result<()> {
        ...
        conn.execute(
            ...
            params![
                entry.content_hash, entry.modality, entry.analysis_json,
                entry.tile_count, entry.token_estimate, entry.created_at, entry.accessed_at,
            ],
        ).map_err(|e| HalconError::DatabaseError(e.to_string()))?;
        Ok(())
    }
    ...
    pub fn store_media_index_entry(&self, entry: &MediaIndexEntry) -> Result<()> {
        let blob: Vec<u8> = entry.embedding.iter().flat_map(|f| f.to_le_bytes()).collect();
        let conn = self.conn()?;
        conn.execute(
    ...
    pub fn search_media_index(
        ...
        let rows = stmt.query_map(params![modality], |row| {
...
mod tests {
    ...
    fn test_db() -> Database { Database::open_in_memory().expect("in-memory DB") }
    ...
    fn media_cache_roundtrip() {
        ...
        let e = MediaCacheEntry {
            content_hash: "abc".to_string(), modality: "image".to_string(),
            analysis_json: r#"{"desc":"cat"}"#.to_string(),
            tile_count: 1, token_estimate: 255,
            created_at: Utc::now().to_rfc3339(), accessed_at: Utc::now().to_rfc3339(),
        };
    ...
    fn media_cache_evict() {
        ...
        db.store_media_cache(&MediaCacheEntry {
            content_hash: "old".to_string(), modality: "audio".to_string(),
            analysis_json: "{}".to_string(), tile_count: 1, token_estimate: 100,
            created_at: old.clone(), accessed_at: old,
        }).unwrap();
    ...
    fn media_index_store_and_search() {
        ...
        db.store_media_index_entry(&MediaIndexEntry {
            id: 0, content_hash: "xyz".to_string(), modality: "image".to_string(),
    ...
    fn media_index_modality_filter() {
        ...
        for (h, m) in [("h1","image"),("h2","audio"),("h3","video")] {
            db.store_media_index_entry(&MediaIndexEntry {
                id: 0, content_hash: h.to_string(), modality: m.to_string(),
                embedding: emb.clone(), embedding_dim: 512,
                clip_start_secs: None, clip_end_secs: None,
                session_id: None, source_path: None,
                created_at: Utc::now().to_rfc3339(),
