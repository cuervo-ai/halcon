...
impl Database {
    ...
    pub fn insert_memory(&self, entry: &MemoryEntry) -> Result<bool> {
        ...
        let exists: bool = conn
            ...
            .map_err(|e| HalconError::DatabaseError(format!("check hash: {e}")))?;
        ...
        let metadata_json = serde_json::to_string(&entry.metadata)
            .map_err(|e| HalconError::DatabaseError(format!("serialize metadata: {e}")))?;
        ...
        .map_err(|e| HalconError::DatabaseError(format!("insert memory: {e}")))?;
    ...
    pub fn load_memory(&self, entry_id: Uuid) -> Result<Option<MemoryEntry>> {
        ...
        let entry = stmt
            ...
            .optional()
            .map_err(|e| HalconError::DatabaseError(format!("load memory: {e}")))?;
    ...
    pub fn search_memory_fts(&self, query: &str, limit: usize) -> Result<Vec<MemoryEntry>> {
        ...
        let mut stmt = conn
            ...
            .map_err(|e| HalconError::DatabaseError(format!("prepare fts: {e}")))?;

        let entries = stmt
            .query_map(rusqlite::params![sanitized, limit as u32], |row| {
                Ok(Self::row_to_memory_entry(row))
            })
            .map_err(|e| HalconError::DatabaseError(format!("search fts: {e}")))?
    ...
    pub fn list_memories(
        ...
        let entries = stmt
            ...
            .map_err(|e| HalconError::DatabaseError(format!("list memories: {e}")))?
            .collect::<std::result::Result<Vec<_>, _>>()
            .map_err(|e| HalconError::DatabaseError(format!("collect: {e}")))?;
    ...
    pub fn delete_memory(&self, entry_id: Uuid) -> Result<bool> {
        ...
        let rows = conn
            ...
            .map_err(|e| HalconError::DatabaseError(format!("delete memory: {e}")))?;
    ...
    pub fn prune_memories(
        ...
        if let Some(cutoff) = expire_before {
            let expired = conn
                ...
                .map_err(|e| HalconError::DatabaseError(format!("prune expired: {e}")))?;
        ...
        if max_entries > 0 {
            let total: u32 = conn
                ...
                .map_err(|e| HalconError::DatabaseError(format!("count: {e}")))?;
            ...
            if total > max_entries {
                ...
                let pruned = conn
                    ...
                    .map_err(|e| HalconError::DatabaseError(format!("prune excess: {e}")))?;
    ...
    pub fn memory_stats(&self) -> Result<MemoryStats> {
        ...
        let (total_entries, oldest_str, newest_str): (u32, Option<String>, Option<String>) = conn
            ...
            .map_err(|e| HalconError::DatabaseError(format!("memory stats: {e}")))?;
        ...
        let by_type: Vec<(String, u32)> = by_type_stmt
            ...
            .map_err(|e| HalconError::DatabaseError(format!("by_type: {e}")))?
            .collect::<std::result::Result<Vec<_>, _>>()
            .map_err(|e| HalconError::DatabaseError(format!("collect: {e}")))?;
    ...
    pub fn update_memory_relevance(&self, entry_id: Uuid, relevance_score: f64) -> Result<bool> {
        ...
        let rows = conn
            ...
            .map_err(|e| HalconError::DatabaseError(format!("update relevance: {e}")))?;
    ...
    pub fn search_memory_fts_by_type(
        ...
        let mut stmt = conn
            ...
            .map_err(|e| HalconError::DatabaseError(format!("prepare fts_by_type: {e}")))?;
        ...
        let entries = stmt
            ...
            .map_err(|e| HalconError::DatabaseError(format!("search fts_by_type: {e}")))?
            .collect::<std::result::Result<Vec<_>, _>>()
            .map_err(|e| HalconError::DatabaseError(format!("collect: {e}")))?;
    ...
    pub fn update_entry_embedding(
        ...
        .map_err(|e| HalconError::DatabaseError(format!("update embedding: {e}")))?;
    ...
    pub fn search_memory_by_embedding(
        ...
        let mut stmt = conn
            ...
            .map_err(|e| HalconError::DatabaseError(format!("prepare embedding search: {e}")))?;
        ...
        let entries_with_embeddings: Vec<(MemoryEntry, Vec<f32>)> = stmt
            ...
            .map_err(|e| HalconError::DatabaseError(format!("embedding search: {e}")))?
            .filter_map(|r| r.ok())
    ...
    pub(crate) fn row_to_memory_entry(row: &rusqlite::Row) -> Result<MemoryEntry> {
        ...
        let session_id = session_id_str
            ...
            .transpose()
            .map_err(|e| HalconError::DatabaseError(format!("parse session uuid: {e}")))?;
        let entry_type = MemoryEntryType::parse(&entry_type_str).ok_or_else(|| {
            HalconError::DatabaseError(format!("unknown memory type: {entry_type_str}"))
        })?;
        let metadata: serde_json::Value = serde_json::from_str(&metadata_json)
            .map_err(|e| HalconError::DatabaseError(format!("parse metadata: {e}")))?;
        ...
        let expires_at = expires_at_str
            ...
            .transpose()
            .map_err(|e| HalconError::DatabaseError(format!("parse expires: {e}")))?
            .map(|dt| dt.with_timezone(&Utc));
