...
fn lookup_cache_ttl_in_where() {
    ...
    let count: u32 = conn
        .query_row(
            "SELECT COUNT(*) FROM response_cache WHERE cache_key = 'ttl_key'",
            [],
            |row| row.get(0),
        )
        .unwrap();
    assert_eq!(
        count, 1,
        "Expired entry should still exist (cleaned by prune_cache)"
    );
}
...
fn cache_with_tool_calls() {
    ...
    entry.tool_calls_json =
        Some(r#"[{"id":"t1","name":"file_read","input":{"path":"a.rs"}}]"#.to_string());
...
fn cache_clear() {
    let db = Database::open_in_memory().unwrap();
    db.insert_cache_entry(&make_cache_entry("k1", "m", "r1"))
        .unwrap();
    db.insert_cache_entry(&make_cache_entry("k2", "m", "r2"))
        .unwrap();
...
fn cache_prune_expired() {
    ...
    db.insert_cache_entry(&make_cache_entry("fresh", "m", "new response"))
        .unwrap();
...
fn cache_prune_excess() {
    let db = Database::open_in_memory().unwrap();
    for i in 0..5 {
        db.insert_cache_entry(&make_cache_entry(&format!("k{i}"), "m", &format!("r{i}")))
            .unwrap();
    }
...
fn memory_stats_consolidated_edge_cases() {
    ...
    // Single entry: oldest == newest.
    db.insert_memory(&make_memory_entry("solo", MemoryEntryType::Fact))
        .unwrap();
    let stats = db.memory_stats().unwrap();
    assert_eq!(stats.total_entries, 1);
    assert!(stats.oldest_entry.is_some());
...
fn metric_insert_and_system_metrics() {
    ...
    db.insert_metric(&make_metric("anthropic", "claude", 500, 0.001, true))
        .unwrap();
    db.insert_metric(&make_metric("anthropic", "claude", 1500, 0.003, true))
        .unwrap();
    db.insert_metric(&make_metric("ollama", "llama", 200, 0.0, true))
        .unwrap();
...
fn model_stats_aggregation() {
    ...
    db.insert_metric(&make_metric("anthropic", "claude", 500, 0.001, true))
        .unwrap();
    db.insert_metric(&make_metric("anthropic", "claude", 1000, 0.002, true))
        .unwrap();
    db.insert_metric(&make_metric("anthropic", "claude", 2000, 0.003, false))
        .unwrap();
...
fn model_stats_p95_latency() {
    ...
    for i in 1..=20 {
        db.insert_metric(&make_metric("a", "m", i * 100, 0.001, true))
            .unwrap();
    }
...
fn system_metrics_group_by_multiple_models() {
    ...
    db.insert_metric(&make_metric("anthropic", "claude-3", 500, 0.01, true))
        .unwrap();
    db.insert_metric(&make_metric("anthropic", "claude-3", 600, 0.02, true))
        .unwrap();
    db.insert_metric(&make_metric("anthropic", "haiku", 100, 0.001, true))
        .unwrap();
    db.insert_metric(&make_metric("ollama", "llama", 200, 0.0, true))
        .unwrap();
    db.insert_metric(&make_metric("ollama", "llama", 300, 0.0, false))
        .unwrap();
...
fn prune_metrics_by_age() {
    ...
    db.insert_metric(&make_metric("a", "m", 200, 0.001, true))
        .unwrap();
...
fn make_resilience_event(provider: &str, event_type: &str) -> crate::resilience::ResilienceEvent {
...
fn query_resilience_events_filters() {
    ...
    db.insert_resilience_event(&make_resilience_event("a", "breaker_trip"))
        .unwrap();
    db.insert_resilience_event(&make_resilience_event("b", "saturation"))
        .unwrap();
    db.insert_resilience_event(&make_resilience_event("a", "recovery"))
        .unwrap();
    ...
    // Filter by type.
    let trips = db
        .resilience_events(None, Some("breaker_trip"), 10)
        .unwrap();
    ...
    // Filter by both.
    let a_recovery = db
        .resilience_events(Some("a"), Some("recovery"), 10)
        .unwrap();
...
fn provider_metrics_windowed_aggregates() {
    ...
    for _ in 0..3 {
        db.insert_metric(&make_metric("prov", "m", 200, 0.001, true))
            .unwrap();
    }
...
fn save_and_load_episode() {
    ...
    assert_eq!(
        loaded.summary,
        Some("Resolved JWT expiry issue".to_string())
    );
...
fn conn_pragma_queries_return_values() {
    ...
    assert!(
        page_size > 0,
        "page_size should be positive (typically 4096)"
    );
...
fn audit_hmac_key_table_exists_after_migration() {
    ...
    assert!(
        table_exists,
        "audit_hmac_key table must exist after migrations"
    );
}
