//! Performance measurement tests for the halcon-storage layer.
//!
//! Run with: cargo test -p halcon-storage --test perf_measurements -- --nocapture
//!
...
fn make_memory_entry(i: usize, entry_type: MemoryEntryType) -> MemoryEntry {
    let content = format!(
        "Memory entry number {} with unique content for testing storage throughput and search. \
         Keywords: rust, async, tokio, sqlite, benchmark, performance, halcon, cli, tool, agent, \
         index-{i}",
        i
    );
