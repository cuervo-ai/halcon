use halcon_client::config::ClientConfig;
use halcon_client::error::ClientError;
...
fn client_creation() {
    ...
    let client = halcon_client::HalconClient::new(config).unwrap();
...
fn client_error_from_api_error() {
    let api_err = halcon_api::error::ApiError::not_found("missing");
    let client_err: ClientError = api_err.into();
    match client_err {
        ClientError::Api(e) => {
            assert_eq!(e.code, halcon_api::error::ErrorCode::NotFound);
        }
