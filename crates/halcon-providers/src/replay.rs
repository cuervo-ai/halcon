...
use halcon_core::types::{ModelChunk, ModelInfo, ModelRequest, StopReason, TokenCost, TokenUsage};
...
impl ReplayProvider {
    ...
    pub fn from_trace(steps: &[halcon_storage::TraceStep], model: &str) -> Result<Self> {
        let mut responses = VecDeque::new();

        for step in steps {
            if step.step_type != halcon_storage::TraceStepType::ModelResponse {
                continue;
            }
...
impl ModelProvider for ReplayProvider {
    ...
    async fn invoke(
        ...
        let response = response.ok_or_else(|| {
            HalconError::Internal("replay trace exhausted: no more recorded responses".into())
        })?;
