//! ContextSource adapter for instruction files (HALCON.md).
...
/// A ContextSource that loads HALCON.md instruction files.
...
mod tests {
    ...
    async fn no_instruction_file_returns_empty() {
        ...
        let chunks = source
            ...
            .await
            .unwrap();
        // May return empty or global HALCON.md content.
        for chunk in &chunks {
    ...
    async fn loads_instruction_file() {
        let dir = TempDir::new().unwrap();
        std::fs::write(dir.path().join("HALCON.md"), "# Rules\nBe concise.").unwrap();
    ...
    async fn source_field_is_instruction() {
        let dir = TempDir::new().unwrap();
        std::fs::write(dir.path().join("HALCON.md"), "content").unwrap();
